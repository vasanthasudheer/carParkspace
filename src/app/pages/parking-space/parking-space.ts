    ...
    let s1 = this.http.GetParkingSpaceById(this.ParkingSpaceID_Ed).subscribe({
      next: (res: IResponse) => {
        ...
        this.parkSpaceForm.patchValue({
          parkingSpaceId: res.data.parkingSpaceId,
          ...
          supportedCarSizeId: res.data.supportedCarSizeId,
...
loadCarSizes() {
 ...
 UpdateFormcontrols() {
  let OwnerID = this.auths.getUserId();
 ...
 OnUpdate() {
  if (this.parkSpaceForm.invalid) {
    this.parkSpaceForm.markAllAsTouched();
    return;
  }
  ...
  const data = {
    ...this.parkSpaceForm.value,
    pricePerMonth: Number(this.parkSpaceForm.value.pricePerMonth)
  };

  let s1 = this.http.UpdateParkingSpace(this.ParkingSpaceID_Ed, data).subscribe({
    next: (res: IResponse) => {
      alert(res.message);
